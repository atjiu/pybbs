package cn.jfinalbbs.collect.model;
...
public class Collect extends Model<Collect> {
    ...
    public Page<Collect> findByAuthorIdWithTopic(int pageNumber, int pageSize, String authorId) {
        return super.paginate(pageNumber, pageSize, "select t.*, u.avatar, (select count(r.id) from reply r where r.tid = t.id) as reply_count",
