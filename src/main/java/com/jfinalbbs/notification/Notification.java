...
public class Notification extends BaseModel<Notification> {
    ...
    public int updateNotification(String uid) {
        return Db.update("update jfbbs_notification n set n.read = 1 where n.author_id = ?", uid);
    }

    //获取未读消息的数量
    public int countNotRead(String uid) {
        return super.find("select n.id as not_read_count from jfbbs_notification n where n.read = 0 and n.author_id = ?", uid).size();
    }
}
