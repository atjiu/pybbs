            ...
            <tr>
              ...
              <td>
                <#if sec.hasPermission('comment:edit')>
                  <a href="/admin/comment/edit?id=${map.comment.id?c}" class="btn btn-sm btn-warning">编辑</a>
                </#if>
                <#if sec.hasPermission('comment:delete')>
                  <button onclick="deleteBtn('${map.comment.id?c}')" class="btn btn-sm btn-danger">删除</button>
                </#if>
              </td>
            </tr>
          </#list>
